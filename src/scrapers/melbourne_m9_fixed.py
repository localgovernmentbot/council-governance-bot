"""
Melbourne City Council Scraper for M9 Bot - Fixed version
"""
...
class MelbourneScraper:
    ...
    def scrape(self):
        ...
        # Melbourne has both PDF links and S3 links
        # Look for all links that might be documents
        for link in soup.find_all('a', href=True):
            href = link.get('href', '')
            text = link.get_text(strip=True)
            
            # Check if it's a document link (PDF or S3)
            is_document = False
            if '.pdf' in href.lower():
                is_document = True
            elif 's3' in href.lower() and 'amazonaws.com' in href.lower():
                is_document = True
            ...
            if is_document:
                ...
                if not any(word in title.lower() for word in ['agenda', 'minutes']):
                    continue
